<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex justify-items space-x-2">
	        <a href="{{ route('courses.chapters.quiz.questions.add', ['course' => $course->id, 'chapter' => $chapter, 'quiz' => $quiz]) }}" class="flex items-center justify-between px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-mohs-green-600 border border-transparent rounded-lg active:mohs-green-600 hover:mohs-green-700 focus:outline-none">
	            Add Question
