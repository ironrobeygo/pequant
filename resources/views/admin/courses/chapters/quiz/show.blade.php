<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex justify-items space-x-2">
	        <a href="{{ route('courses.chapters.quiz.questions.add', ['course' => $course->id, 'chapter' => $chapter, 'quiz' => $quiz]) }}" class="flex items-center justify-between px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-pequant-blue-600 border border-transparent rounded-lg active:pequant-blue-600 hover:pequant-blue-700 focus:outline-none">
	            Add Question
    ...
    @livewire('courses.chapters.quiz.show', ['course' => $course, 'chapter' => $chapter, 'quiz' => $quiz])
</x-app-layout>
